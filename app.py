...
# 單字：家庭成員 (全部改為小寫，Wama->mama, Wina->ina)
VOCABULARY = {
    "mama":     {"zh": "爸爸", "emoji": "👨", "file": "u3_wama"},
    "ina":      {"zh": "媽媽", "emoji": "👩", "file": "u3_wina"},
...
# 句型：更新為小寫與新稱謂
SENTENCES = [
    {"amis": "Romadiw ci ina.", "zh": "媽媽在唱歌。", "file": "u3_s_mom_sings"},
...
def show_learning_mode():
    ...
    st.warning("A: Ci ina. (是媽媽。)")
    play_audio("Ci ina", filename_base="u3_wina")
    ...
    if st.session_state.current_q == 0:
        ...
        with c1:
            if st.button("👴 akong"):
                ...
                st.rerun()
        with c2:
            if st.button("👵 ama"): st.error("那是 ama 喔！")

    elif st.session_state.current_q == 1:
        st.markdown("### 第二關：誰在唱歌？")
        st.markdown("#### 請聽句子：")
        play_audio("Romadiw ci ina.", filename_base="u3_s_mom_sings")
        ...
        with c1:
            if st.button("👩 媽媽 (ina)"):
                st.snow()
                st.success("沒錯！ Romadiw ci ina.")
                time.sleep(1)
                st.session_state.score += 100
                st.session_state.current_q += 1
                st.rerun()
        with c2:
            if st.button("👶 妹妹 (safa)"): st.error("不對喔！")
        ...
        options = ["Ci mama (是爸爸)", "Ci akong (是阿公)", "Ci safa (是弟弟)"]
